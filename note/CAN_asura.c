...
void can_put_receved_data(uint8_t data_type,uint8_t data_symbol, long CANRx_Data)
{
	...
	switch(data_type){//データの種類で振り分け
		...
		case com_ballscllew_1_pos ://actuator_1's command
				...
				///指令値はストロークの100倍の値
				command_stroke.stroke_act1 =CANRx_Data; //command_stroke.stroke_act1 =CANRx_Data
				CANRx_Data=RESET;
		...
		case encoder_1_count ://ゲートウェイへエンコーダ1の情報を送信する
			...
			CANRx_Data=(uint32_t)encoder_count.enccount1;//<--BAD CANRx_Data=(uint32_t)encoder_count.enccount1;
